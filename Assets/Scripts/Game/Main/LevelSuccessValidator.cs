...
namespace DLS.Game
{
    public static class LevelSuccessValidator
    {
        ...
        public static bool CheckLevel6_Register(DevChipInstance circuit)
        {
            // 1. Component Count and Type Check
            var allDevPins = circuit.Elements.OfType<DevPinInstance>().ToList();
            ...
            // A Register requires 2 single-bit inputs (D, E/Clock) and 1 single-bit output (Q).
            if (devInputPins.Count != 2 || devOutputPins.Count != 1)
            {
                return false;
            }

            // 2. Pin Mapping (Identify Data, Enable/Clock, and Output)
            // We will attempt to find the pins by the common names: D (Data), E (Enable) or Clk (Clock).
            DevPinInstance dPin = devInputPins.FirstOrDefault(p => p.Name.Equals("D"));
            DevPinInstance ePin = devInputPins.FirstOrDefault(p => p.Name.Equals("E") || p.Name.Equals("Clk"));

            // Fallback to array indices (assuming standard setup: D, E/Clk):
            if (dPin == null) dPin = devInputPins[0];
            if (ePin == null || ePin == dPin) 
            {
                // Ensure E is the other pin if not found by name, or if it accidentally matched D.
                ePin = devInputPins.First(p => p != dPin);
            }
            
            DevPinInstance qPin = devOutputPins[0]; // The single output pin

            // Safety check: ensure D and E pins are distinct and non-null
            if (dPin == null || ePin == null || dPin == ePin) return false;


            // 3. Truth Table and Sequential Logic Check
            
            // A Register's behavior is dependent on its previous state. We must run a sequence of simulations.
            // The PinState needs to be initialized before the first simulation run.
            int currentQ = 0; // Initialize Q to 0 for a known starting state.

            // Helper to run a test and update the expected Q state for the next step.
            int RunAndCheck(int dVal, int eVal, int expectedQ)
            {
                int actualQ = RunSimulationTest(circuit, dPin, ePin, qPin, dVal, eVal, 0); // Note: We use the 3rd pin slot d0Pin as qPin to run the simulation
                
                // PinState.GetBitStates returns the lowest 16 bits of the state, which for a 1-bit pin is the 0 or 1 value.
                // The RunSimulationTest is set up to use d0Pin slot to pass the qPin in, but it's not being used as an input.
                // It's a bit of a hack in RunSimulationTest to use D1 and D0 slots for E and Q.
                // We'll re-map for clarity in the simulation call, assuming RunSimulationTest expects S, D1, D0 in order.
                // Let's assume the order we pass to RunSimulationTest is: (D, E, Unused Output Q).
                // Since the RunSimulationTest is hardcoded to use sPin, d1Pin, d0Pin for inputs, we'll map them:
                // sPin maps to D, d1Pin maps to E, d0Pin is unused as an input, but the output check is done on circuit.SimChip.OutputPins[0].
                // RunSimulationTest(circuit, sPin, d1Pin, d0Pin, sVal, d1Val, d0Val) -> s=D, d1=E, d0=unused input value 0

                actualQ = RunSimulationTest(circuit, dPin, ePin, qPin, dVal, eVal, 0);
                
                if (actualQ != expectedQ)
                {
                    // Failed: D={dVal}, E={eVal}. Expected Q={expectedQ}, Actual Q={actualQ}
                    return -1; // Indicate failure
                }
                return actualQ; // Return the new state (which is the actual Q)
            }

            // Step 1: E=1 (Enable), D=1. Q should become 1.
            currentQ = RunAndCheck(dVal: 1, eVal: 1, expectedQ: 1);
            if (currentQ == -1) return false;
            
            // Step 2: E=0 (Hold), D=0. Q should remain 1.
            currentQ = RunAndCheck(dVal: 0, eVal: 0, expectedQ: 1);
            if (currentQ == -1) return false;

            // Step 3: E=1 (Enable), D=0. Q should become 0.
            currentQ = RunAndCheck(dVal: 0, eVal: 1, expectedQ: 0);
            if (currentQ == -1) return false;

            // Step 4: E=0 (Hold), D=1. Q should remain 0.
            currentQ = RunAndCheck(dVal: 1, eVal: 0, expectedQ: 0);
            if (currentQ == -1) return false;

            // Step 5: E=1 (Enable), D=1 again. Q should become 1. (Test setting back to 1)
            currentQ = RunAndCheck(dVal: 1, eVal: 1, expectedQ: 1);
            if (currentQ == -1) return false;

            // Step 6: E=0 (Hold), D=1 again. Q should remain 1. (Test holding a 1)
            currentQ = RunAndCheck(dVal: 1, eVal: 0, expectedQ: 1);
            if (currentQ == -1) return false;

            // If all sequential checks passed, the logic is correct.
            return true;
        }
    }
}